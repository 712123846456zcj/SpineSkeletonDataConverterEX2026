...
if len(sys.argv) != 2:
    print('usage: regression_300_s27_hash.py <converter-exe>')
    sys.exit(2)

exe = Path(sys.argv[1])
repo = Path(__file__).resolve().parent.parent
input_file = repo / '300_s27.json'
output_file = repo / '300_s27.hash-check.json'
...
if result.returncode != 0:
    print('UNEXPECTED FAILURE: 2.1 -> 3.8 conversion should succeed')
    sys.exit(1)
...
if not isinstance(out_hash, str) or not out_hash:
    print('EXPECTED RED: 2.1 -> 3.8 output is missing skeleton.hash')
    sys.exit(1)

print('PASS: 2.1 -> 3.8 output contains skeleton.hash')
